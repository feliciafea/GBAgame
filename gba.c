...
void undrawImageDMA(int row, int col, int width, int height, const u16 *image) {
  // TODO: IMPLEMENT
    for(int r=0; r<height; r++) {
          DMA[3].src = (image + OFFSET(row+r,col,SCREEN_WIDTH) + (r*width));
          DMA[3].dst = &videoBuffer[OFFSET(row+r,col,SCREEN_WIDTH)];
          DMA[3].cnt = width | DMA_ON | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT | width;
  }
  UNUSED(row);
  UNUSED(col);
  UNUSED(width);
...
void fillScreenDMA(volatile u16 color) {
  ...
  for(int r=0; r<SCREEN_HEIGHT; r++) {
          ...
          DMA[3].cnt = SCREEN_WIDTH | DMA_ON | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT;
  }
}
